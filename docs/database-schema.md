...
CREATE TABLE session_requests (
    ...
    pricing_model_snapshot VARCHAR(16) NOT NULL CHECK (pricing_model_snapshot IN ('HOURLY', 'FLAT')),
    hourly_rate_cents_snapshot INT,
    flat_rate_cents_snapshot INT,
    quoted_amount_cents INT NOT NULL CHECK (quoted_amount_cents >= 0),
    payment_due_at TIMESTAMP,
    status VARCHAR(16) NOT NULL
        CHECK (status IN ('REQUESTED', 'APPROVED_PENDING_PAYMENT', 'APPROVED_PAID', 'DECLINED', 'CANCELLED', 'EXPIRED', 'COMPLETED')),
...
CREATE TABLE mentor_pricing_settings (
    mentor_user_id BIGINT PRIMARY KEY,
    default_pricing_model VARCHAR(16) NOT NULL CHECK (default_pricing_model IN ('HOURLY', 'FLAT')),
    hourly_rate_cents INT,
    flat_rate_cents INT,
    currency CHAR(3) NOT NULL DEFAULT 'CAD',
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_mentor_pricing_profile FOREIGN KEY (mentor_user_id) REFERENCES mentor_profiles(user_id)
);

CREATE TABLE mentor_slot_locks (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    mentor_user_id BIGINT NOT NULL,
    slot_start_at TIMESTAMP NOT NULL,
    slot_end_at TIMESTAMP NOT NULL,
    session_request_id BIGINT NOT NULL UNIQUE,
    lock_state VARCHAR(16) NOT NULL CHECK (lock_state IN ('HELD', 'CONFIRMED')),
    expires_at TIMESTAMP,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_slot_lock UNIQUE (mentor_user_id, slot_start_at, slot_end_at),
    CONSTRAINT fk_slot_lock_mentor FOREIGN KEY (mentor_user_id) REFERENCES users(id),
    CONSTRAINT fk_slot_lock_request FOREIGN KEY (session_request_id) REFERENCES session_requests(id)
);
...
CREATE TABLE session_payments (
    ...
    currency CHAR(3) NOT NULL DEFAULT 'CAD',
    status VARCHAR(20) NOT NULL CHECK (status IN ('NOT_STARTED', 'PENDING', 'PAID', 'PARTIAL_REFUND', 'REFUNDED', 'FAILED')),
    provider VARCHAR(40),
...
CREATE INDEX idx_mentor_slot_locks_active
    ON mentor_slot_locks(mentor_user_id, slot_start_at, lock_state, expires_at);
CREATE INDEX idx_session_payments_status_due
    ON session_payments(status, payment_due_at);
...
- Create `session_requests` row with `status='REQUESTED'` and pricing snapshots.
- Create `mentor_slot_locks` row in `HELD` state with 24h expiry.
- Add first `session_status_history` row.

7. Mentor decision (`POST /mentor/requests/{id}/decision`)
- Approve: set status to `APPROVED_PENDING_PAYMENT`, set lock to `CONFIRMED`.
- Decline: set status to `DECLINED`, release slot lock.
- Save `mentor_note` and append `session_status_history`.

8. Mentee payment (`POST /seeker/sessions/{requestId}/payment`)
- Update `session_payments.status` to `PAID` and session status to `APPROVED_PAID`.

9. Mentee/Mentor calendar + upcoming cards
- Read from `session_requests` by role where status in active statuses and slot date range.

10. Admin mentor review (`/admin/mentors/review`)
...
FROM session_requests
WHERE mentor_user_id = :mentor_user_id
  AND status IN ('APPROVED_PENDING_PAYMENT', 'APPROVED_PAID', 'COMPLETED')
